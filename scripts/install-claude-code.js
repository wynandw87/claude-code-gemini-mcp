...
/**
 * Helper script to install Gemini MCP Server in Claude Code
...
console.log('🚀 Installing Gemini MCP Server for Claude Code\n');
...
try {
  ...
  if (output.includes('gemini')) {
    isInstalled = true;
    console.log('⚠️  Gemini MCP server is already installed in Claude Code');
    console.log('   To reinstall, first run: claude mcp remove gemini\n');
  }
} catch (error) {
...
if (!isInstalled) {
  try {
    ...
    // Build command based on whether API key is available
    let command;
    if (process.env.GEMINI_API_KEY) {
      console.log('🔑 Using API key from environment...');
      command = `claude mcp add gemini -e GEMINI_API_KEY="${process.env.GEMINI_API_KEY}" -- node "${absolutePath.replace(/\\/g, '/')}"`;
    } else {
      command = `claude mcp add gemini -- node "${absolutePath.replace(/\\/g, '/')}"`;
    }
    ...
    console.log('   Restart Claude Code to use the Gemini MCP server\n');
  } catch (error) {
    console.error('❌ Installation failed:', error.message);
    console.error('\nManual installation:');
    console.error(`   claude mcp add gemini -- node "${absolutePath.replace(/\\/g, '/')}"`);
